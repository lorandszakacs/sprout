...
trait OldType[O, N] {
  ...
  /** Ought to be used only for pretty printing and debug messages, not intented to represent extremly precise and
    ...
    *
    * @return
    *   e.g. `` object TestValue extends Sprout[Int] //which extends OldType type TestValue = TestValue.Type `` returns
    *   TestValue
    */
...
object OldType {
  @inline def apply[O, N](using i: OldType[O, N]): OldType[O, N] = i
}
